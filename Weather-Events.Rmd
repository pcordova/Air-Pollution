...
We are going to do some configuration, download and sub-setting data file, etc.
```{r}
...
cost[order(-Fatalities,-Injuries,-propLoss,-cropLoss)] %>% 
    datatable(colnames = c('Property Loss' = 4, 'Crop Loss' = 5),
              caption = "Total Weather Events in the U.S., from Deadliest to less Harmful (1950-2011)",
              rownames = FALSE,
              options = list(dom = 'tp')) %>%
...
```{r}
cat("Deadliest weather event is", paste0(cost[which.max(cost$Fatalities),Event],"."),
    "\nIt accounts for",cost[,max(Fatalities)],"fatalities between 1950 and 2011.")

cat("Weather event causing more injuries in the same period (1950-2011) is",
